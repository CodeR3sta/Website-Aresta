...
let usersImage = (req, res) => {
    return res.sendFile( path.join(__dirname,'..','upload',`${req.params.username}${req.params.sekolah}`,`${req.params.gambar}`))
}
        ...
        if (user) {
          ...
          if (match === true) {
            ...
            if (req.body.hasil === 'benar') {
                ...
                db.query(`UPDATE users SET status = 1, pesan = '${pesan}',kis = '',suratRekomendasi = '',fotoTim = '',postIg = '', fotoDiri = '' WHERE id = ?`,[req.params.id],(err, results) => {
                    if(err){
                        console.log(err)
                        return res.redirect('/code/resta/panitia/users')
                    }
                    ...
                    db.query('SELECT * FROM users WHERE id = ?',[req.params.id],(err,results) => {
                        ...
                        rmdirSync(path.join(__dirname,'..','upload',`${results[0].username}${results[0].sekolah}`),{recursive : true},(err) => {
                            if(err){
                                console.log(err)
                                return res.redirect('/code/resta/panitia/users')
                            }

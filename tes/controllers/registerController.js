...
let createNewUser = async (req,res) => {
    ...
    try {
        ...
        return res.send(`<h1 style="margin:50px auto;padding: 10px;width:80%;background-color: #0db02b;color:white;text-align:center;border-radius:10px;">Berhasil Registrasi, Silahkan Cek email Anda Untuk Mengaktifkan Akun Anda</h1>`)
    ...
    try {
        db.query('SELECT users.verification FROM users WHERE email = ?',req.cookies.userInfo.email,(err,result) => {
            ...
            if (req.query.verify === result[0].verification) {
                ...
                return res.send(`<div style="margin:50px auto;padding: 10px;width:80%;background-color: red;color:white;text-align:center;border-radius:10px;"><h1>Gagal Verifikasi Akun</h1></div>`)
            }
        })
    } catch (error) {
        return res.send(`<div style="margin:50px auto;padding: 10px;width:80%;background-color: red;color:white;text-align:center;border-radius:10px;"><h1>Silahkan daftar Terlebih Dahulu</h1></div>`)
